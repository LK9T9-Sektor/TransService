...
then
showmessage('Данный прицеп задействован, изменить невозможно!!!')
else begin
...
if (TAvto.ADOTable1.Locate('gosnomer_pricepa', TPR.ADOTable1gosnomer_pricepa.Text, [LoCaseInsensitive, loPartialKey])=True)
 then
Showmessage('Данный прицеп связан с автомобилями! Удалить невозможно!')
else
...
then showmessage('Данный прицеп задействован, удалить невозможно')
else
if messagedlg('Вы действительно хотите удалить запись?',mtconfirmation,[mbyes,mbno],0)=mryes
then
if (TAvto.ADOTable1.Locate('gosnomer_pricepa', TPr.ADOTable1gosnomer_pricepa.Text, [LoCaseInsensitive, loPartialKey])=True)
 then
if messagedlg('Данный прицеп закреплен за автомобилем!!! Вы действительно хотите удалить запись?  ',mtconfirmation,[mbyes,mbno],0)=mryes
then begin
